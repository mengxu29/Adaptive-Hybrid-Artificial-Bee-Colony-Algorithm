function ApplicationExample_1_bootstrap_P_KS_test_GMWeibull()
global lifes;
...
Func_Paras.ub = [100 100 100 100];%[1.9 10 max_eta]; %/*lower bounds of the parameters. */
...
ABC_Paras.limit=200; %/*A food source which could not be improved through "limit" trials is abandoned by its employed bee*/
...
save data_ApplicationExample_1_bootstrap_P_KS_test_GMWeibull
...
neg_loglik_vector = -(log(beta)+log(theta)+(gamma-1).*log(t)+log(gamma+lamda*t)+(lamda*t-beta*(t.^gamma).*exp(lamda*t))+(theta-1)*log(1-exp(-beta.*(t.^gamma).*exp(lamda*t))));
...
lamda = paras(3);
u = rand(1,n);
samples = (((1-log(1-u)).^lamda-1)/alpha).^(1/theta);

function b = isFeasible(paras)
b = paras(1)>0 && paras(2)>0 && paras(3)>0 && paras(4);
