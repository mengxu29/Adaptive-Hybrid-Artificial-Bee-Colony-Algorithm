function ApplicationExample_1_bootstrap_P_KS_test_GeneralizedPower()
global lifes;
...
save data_ApplicationExample_2_bootstrap_P_KS_test_GeneralizedPower
...
neg_loglik_vector = -(log(alpha)+log(theta)-log(lamda)+(theta-1)*log(t)+((1/lamda)-1)*log(1+alpha*t.^theta)+1-(1+alpha*t.^theta).^(1/lamda));
