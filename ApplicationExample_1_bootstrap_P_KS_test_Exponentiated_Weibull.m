function ApplicationExample_1_bootstrap_P_KS_test_Exponentiated_Weibull()
global lifes;
...
Func_Paras.ub = [100 10 100];%[1.9 10 max_eta]; %/*lower bounds of the parameters. */
...
for jj = 2:1000
    ...
    LifeData{jj} = randSampleFunc(sol,n);
end
...
save data_ApplicationExample_1_bootstrap_P_KS_test_Exponentiated_weibull
...
% because log(1-exp(-x)) will go to -Inf, if x is realy small, e.g. 1e-50
% in this case, we use the approximation log(1-exp(-x)) = log(x) instead
special_term = (theta-1)*log(1-exp(-(t/sigma).^alpha)).*((t/sigma).^alpha>=1e-10)...
                + (theta-1)*log((t/sigma).^alpha).*((t/sigma).^alpha<1e-10);

neg_loglik_vector = -(log(alpha)+log(theta)-log(sigma)+special_term-(t/sigma).^alpha+(alpha-1)*log(t/sigma));
neg_loglik = sum(neg_loglik_vector);

if (neg_loglik == -Inf)
    display('something wrong with the CONSTRAINTS');
end
...
sigma = paras(3);
u = rand(1,n);
samples = sigma*(-log(1-u.^(1/theta))).^(1/alpha);
