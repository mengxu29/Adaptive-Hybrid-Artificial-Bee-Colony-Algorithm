function ApplicationExample_2_bootstrap_P_KS_test_Lemonte()
global lifes;
...
Func_Paras.ub = [100 100 100];%[1.9 10 max_eta]; %/*lower bounds of the parameters. */
...
save data_ApplicationExample_2_BCI_P_ENH_Lemonte
...
neg_loglik_vector = -(log(alpha)+log(beta)+log(lamda)+(alpha-1).*log(1+lamda*t)+1-(1+lamda*t).^alpha-(1-beta).*log(1-exp(1-(1+lamda*t).^alpha)));
...
lamda = paras(3);
u = rand(1,n);
samples = ((1-log(1-u.^(1/beta))).^(1/alpha)-1)./lamda;
