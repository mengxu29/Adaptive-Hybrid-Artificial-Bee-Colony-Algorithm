...
load data_ApplicationExample_1_BCI_P_Hjorth;
...
t = LifeData{1};
cdf = 1-(exp(-solutions(1,1)*(t.^2)/2))./((1+solutions(1,2)*t).^(solutions(1,3)/solutions(1,2)));
cdf = sort(cdf);
...
for jj = 2:1000
    ...
    t = LifeData{jj};
    cdf = 1-(exp(-solutions(jj,1)*(t.^2)/2))./((1+solutions(jj,2)*t).^(solutions(jj,3)/solutions(jj,2)));
    cdf = sort(cdf);
    Dmais = max(abs(Fn(2:n+1,1) - cdf));
    Dmenos = max(abs(Fn(1:n,1) - cdf));
