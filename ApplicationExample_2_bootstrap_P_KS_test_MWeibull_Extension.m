function ApplicationExample_2_bootstrap_P_KS_test_MWeibull_Extension()
global lifes;
...
Func_Paras.ub = [10 1 1];%[1.9 10 max_eta]; %/*lower bounds of the parameters. */
...
save data_ApplicationExample_2_BCI_P_MWeibull_Extension_1e5
...
neg_loglik_vector = -(log(beta)+log(lamda)+(beta-1)*log(t/alpha)+(t/alpha).^beta+alpha*lamda*(1-exp((t/alpha).^beta)));
...
lamda = paras(3);
u = rand(1,n);
samples = alpha*((log(1-(log(u)/(alpha*lamda)))).^(1/beta));
